/**
 * @file fast_fifo.c
 * @brief Source code for a (software) circular buffer
 *
 * @copyright @parblock
 ...
 * thread independence... */
fifo_type F_FIFO_MaxWords(F_FIFO_t *p_fifo)
{
    return p_fifo->buffer_words - 1;
}

/* 'Thread safe' for reading. */
bool F_FIFO_IsEmpty(F_FIFO_t *p_fifo)
{
    return p_fifo->read_i==p_fifo->write_i;
}

/* 'Thread safe' for writing. */
bool F_FIFO_IsFull(F_FIFO_t *p_fifo)
{
...
fifo_type F_FIFO_WordsFull(F_FIFO_t *p_fifo)
{
...
fifo_type F_FIFO_WordsEmpty(F_FIFO_t *p_fifo)
{
    return p_fifo->buffer_words-1 - F_FIFO_WordsFull(p_fifo);
}

#if DEBUG_FIFO_BUFFER
/* Read and reset the FIFO's high watermark. */
fifo_type F_FIFO_HighWatermark(F_FIFO_t *p_fifo)
{
    fifo_type max_buffer_words = p_fifo->max_buffer_words;

    p_fifo->max_buffer_words = F_FIFO_WordsFull(p_fifo);
 ...
 * ( == FIFO_WORDS). */
bool F_FIFO_Init(F_FIFO_t *p_fifo, fifo_type fifo_words, word_type *fifo_bufptr)
{
    p_fifo->write_i      = 0;
    p_fifo->read_i       = 0;
    ...
    if (fifo_words && !(fifo_words & (fifo_words-1)))
    {
        p_fifo->buffer_mask = fifo_words - 1;
    }
    else
    {
        /* fifo_words is not a power of two! */
        return false;
    }
...
void F_FIFO_Reset(F_FIFO_t *p_fifo)
{
    p_fifo->write_i = 0;
    p_fifo->read_i  = 0;
}
 ...
 * 'Thread safe' for writing. */
void F_FIFO_EnqueueWord(F_FIFO_t *p_fifo, word_type word)
{
    p_fifo->p_buffer[p_fifo->write_i++] = word;             /* write
                                                             * increment index */

    p_fifo->write_i &= p_fifo->buffer_mask;                 /* wrap index */

#if DEBUG_FIFO_BUFFER
    if (F_FIFO_WordsFull(p_fifo)>p_fifo->max_buffer_words)
    {
        p_fifo->max_buffer_words = F_FIFO_WordsFull(p_fifo);
    }
#endif
 ...
 * Doesn't check for buffer fullness! */
void F_FIFO_EnqueueWords(F_FIFO_t *p_fifo, word_type *wordp, fifo_type n)
{
 ...
 * 'Thread safe' for reading. */
word_type F_FIFO_DequeueWord(F_FIFO_t *p_fifo)
{
    word_type word = p_fifo->p_buffer[p_fifo->read_i++];    /* read
                                                             * increment index */

    p_fifo->read_i &= p_fifo->buffer_mask;                  /* wrap index */
...
fifo_type F_FIFO_DequeueWords(F_FIFO_t *p_fifo, word_type *wordp, fifo_type n)
{
    fifo_type words_in_fifo    = F_FIFO_WordsFull(p_fifo);
...
word_type F_FIFO_GetWord(F_FIFO_t *p_fifo, fifo_type n)
{
...
fifo_type F_FIFO_DumpWords(F_FIFO_t *p_fifo, fifo_type n)
{
    fifo_type words_in_fifo  = F_FIFO_WordsFull(p_fifo);
